...
//manejamos las paginas con argmento dinamico [name] en el build nos dice cuales son todos los argumentos dinamicos permitidos 
//para entrar en esta página por nombre
export const getStaticPaths: GetStaticPaths = async (ctx) => {

    //almacenamos la data de cada uno de los pokemons misma peticion que en pages/index.jsx
    const { data } = await pokeAPi.get<PokemonListResponse>('/pokemon?limit=151');

    //obtenemos de la data los results y de ellos recorremos con map para obtener el name de cada uno de los pokemons
    const pokemonNames: string[] = data.results.map(pokemon => pokemon.name);

    return {
        //la cantidad de paths que tengamos es la cantidad de paginas que vamos a generar en build time
        //pasamos en el paths el array creado arriba pokemonsName
...
//generamos los props que tendra esta pagina, le pasamos los props esta pagina PokemonByNamePage manejada como [name].tsx para generar paginas dinamicamente
//en el build todas las paginas han sido cargadas en el lado del servidor
export const getStaticProps: GetStaticProps = async (ctx) => {

    //leemos el ctx el contexto para recibir los parametros ctx.params donde obtenemos el name de la pagina del pokemon seleccionado
...
export default PokemonByNamePage; //importante usamos la exportacion por defecto
